...
const RegisterView = () => {
    ...
    const registerInputsData = [
        {name: "name", type:"text", placeHolder: "Nombre", icon: <BiUserPlus />},
    ...
    return (
        <>
        <h3>Registrarse</h3>
        <motion.form initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className={styles.registerForm} onSubmit={formik.handleSubmit}>
            {
            ...
            }
            {
                loading ? <Loading /> :
                <>
                    <p><Link to="/auth/login">¿Ya sos usuario? Ingresa </Link></p>
